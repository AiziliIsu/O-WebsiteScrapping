...
# CONFIG
# =========================
OUTPUT_DIR = Path(r"C:\Users\1000001392\.vscode\RQ1 draft\dataset\kyrgyz")
...
def normalize_scope_path(path: str) -> str:
    """
    o.kg has parallel prefixes:
      /ru/chastnym-klientam/... <-> /ru/...
      /kg/chastnym-klientam/... <-> /kg/...
      /en/chastnym-klientam/... <-> /en/...
    Normalize them to avoid excluding valid children during subtree checks.
    """
    if not path:
        return "/"
    path = re.sub(r"/{2,}", "/", path)
...
def page_looks_interactive(soup: BeautifulSoup) -> bool:
    ...
    if soup.select_one(".accordion, .faq, .tabs, .tab, .collapse, .spoiler, .toggle"):
        return True
    if soup.find(string=re.compile(r"(подробнее|далее|раскрыть|показать|more|read|көбүрөөк|толугураак|ачуу)", re.I)):
        return True
    return False
    ...
    for _ in range(rounds):
        clicked = page.evaluate(
            ...
            () => {
              ...
              const textKw = /(more|read|details|expand|show|load|далее|подробнее|раскрыть|показать|загрузить|көбүрөөк|толугураак|ачуу)/i;
...
# =========================
# PHASE 1: DISCOVER URLS ONLY (IN MEMORY)
# =========================
...
# =========================
# PHASE 2: SCRAPE URLS ONE BY ONE + STREAM SAVE (JSONL ONLY)
# =========================
def infer_category_name(root_url: str, root_page: Optional[PageData]) -> str:
    if root_page and root_page.breadcrumbs:
        for candidate in reversed(root_page.breadcrumbs):
            if candidate and candidate.lower() not in {"башкы", "главная", "home"}:
                return candidate
...
def run(root_url: str) -> None:
    ...
    # Kyrgyz preference
    accept_language = "ky-KG,ky;q=0.9,ru-RU;q=0.7,ru;q=0.6,en;q=0.4"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Example: python scrape_one_category_kg.py https://o.kg/kg/chastnym-klientam/")
        raise SystemExit(2)
